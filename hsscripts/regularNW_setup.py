...
# nw_type = "KG_NPT"		# 密度、末端間距離を設定値に合わせるように多重度を変化。 絡み合いが入らないようにNPTで縮める
#
...
############
	...
	##################
	# baseUDF の作成
	baseudf = modules_1.SetupInitUDF.MakeInitUDF(sim_cond, target_cond, files_cond, names, target_name, calcd_data_dic, expand, condition_text)
	target_dir = baseudf.setup_baseudf()
	###############
	# 
	setup = modules_1.EquivCalcSetup.SetUpUDF(nw_type, files_cond, target_name, target_dir, names)
	setup.setup_udf()
